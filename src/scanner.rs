...
pub fn scan_triangles(prices: &[PairPrice], min_profit: f64, fee_per_leg: f64) -> Vec<TriangularResult> {
        ...
        for b in bs {
            ...
            if let Some(cs) = neighbors.get(b) {
                for c in cs {
                    ...
                    // lookup rates (we inserted direct and inverses)
                    ...
                    // sanity checks: finite, positive, not absurd (>10%)
                    if !profit_before.is_finite() || profit_before <= 0.0 || profit_before > 10.0 { continue; }
                    ...
                    // canonical dedupe key (lexicographically smallest rotation)
                    let reps = vec![
                    ...
                    out.push(TriangularResult {
                        ...
                        profit_before_fees: crate::utils::round2(profit_before),
                        trade_fees: crate::utils::round2(total_fee_percent),
                        profit_after_fees: crate::utils::round2(profit_after),
                    });
                }
            }
        }
    ...
    out.sort_by(|x,y| y.profit_after_fees.partial_cmp(&x.profit_after_fees).unwrap_or(std::cmp::Ordering::Equal));
    out
    }
